...
export def gen-stats-page [
    ...
    let qrcode = $qrcode | ffmpeg apply $"scale=($options.qr_code_width):($options.qr_code_width)" -o /tmp/@rand.png
    ...
    let tmp = ffmpeg create ($options.base_image | ffmpeg options) -o /tmp/@rand.png
        ...
        | [$in, $qrcode] | ffmpeg combine ($options.qr_code_overlay | ffmpeg options) -o /tmp/@rand.png
        | ffmpeg mapply ($transforms | compact | each { ffmpeg options }) -o /tmp/@rand.png
